...
task("deploy-token", "Deploys a BurnMintERC677 token and optionally mints initial tokens")
  ...
  .setAction(async (taskArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    try {
      ...
      await verifyContract(hre, tokenAddress, [
        name,
        symbol,
        tokenDecimals,
        hre.ethers.parseUnits(supply, tokenDecimals).toString()
      ]);
    } catch (error) {
      console.log(`⚠️  Token verification failed but deployment was successful. You can verify manually later.`);
    }
...
task("setup-burn-mint-pool", "Deploys and sets up a BurnMintTokenPool")
  ...
  .setAction(async (taskArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    // Verify the deployed pool contract
    try {
      ...
      console.log(`⚠️  Pool verification failed but deployment was successful. You can verify manually later.`);
    }
