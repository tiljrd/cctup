...
export function ExecutionStep({ wizardData, setWizardData }: SimulateExecutionStepProps) {
  ...
  // Find forked networks that correspond to the selected chains
  // If user selected "sepolia", we want to deploy to "sepoliaFork"
  const selectedForkedNetworks = forkedNetworks.filter(forkedNetwork => {
    // Check if this forked network's 'forkedFrom' matches any of our selected chains
    return wizardData.selectedChains.some(selectedChain => {
      const existingNetwork = existingNetworks.find(n => n.key === selectedChain);
      return existingNetwork && existingNetwork.fork === forkedNetwork.key;
    });
  });
  ...
  useEffect(() => {
    ...
    if (!deploymentStarted && !wizardData.deploymentStarted && forkedChainKeys.length > 0) {
      console.log('🚀 Initiating deployment process to forked networks:', forkedChainKeys);
  ...
  const deployMultiChain = async () => {
    try {
      console.log('🚀 Starting multi-chain deployment to forked networks...');
      
      // Initialize results for all forked chains
      const initialResults = forkedChainKeys.reduce((acc, chainKey) => {
        acc[chainKey] = { 
      ...
      // Deploy to each forked chain
      const deploymentPromises = forkedChainKeys.map(async (chainKey) => {
        try {
          console.log(`🚀 Starting deployment to forked network ${chainKey}...`);
          ...
          const response = await fetch('/api/hardhat/multi-chain-deploy', {
            ...
            body: JSON.stringify({
              chainKey, // Deploy to forked network
              tokenConfig: wizardData.tokenConfig,
              poolType: wizardData.poolType,
          ...
          if (result.success) {
            console.log(`✅ Successfully deployed to forked network ${chainKey}`);
            ...
            // Initialize configuration status for all other forked chains
            forkedChainKeys.forEach(targetChain => {
      ...
      if (successfulDeployments >= 2) {
        ...
        await configureNetworks(localResults, forkedChainKeys);
      } else {
  ...
  // Update configureNetworks to accept forkedChainKeys
  const configureNetworks = async (localResults: WizardData['deploymentResults'], chainKeys: string[]) => {
    try {
      const successfulChains = chainKeys.filter(
  ...
  const startVerification = async (currentWizardData: WizardData) => {
    ...
    const successfulChains = forkedChainKeys.filter(
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-4 sm:p-6 lg:p-8">
      <div className="mb-6 sm:mb-8">
        <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Execute Simulated Transactions (in Forked Networks)</h2>
        <p className="text-gray-600 text-sm sm:text-base">
          {currentPhase === 'deploying' && 'Deploying tokens and pools across forks of all selected chains...'}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
        {selectedForkedNetworks.map(network => {
          const result = wizardData.deploymentResults[network.key];
          const configStatus = result?.configurationStatus || {};
          const totalTargets = forkedChainKeys.filter(c => c !== network.key).length;
                ...
                <div className="border-t border-gray-200 pt-4">
                  ...
                  <div className="space-y-1">
                    {forkedChainKeys
                      ...
                      .map(targetChain => {
                        const targetNetwork = selectedForkedNetworks.find(n => n.key === targetChain);
...
export function SimulateExecutionStep({ wizardData, setWizardData }: SimulateExecutionStepProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-4 sm:p-6 lg:p-8">
      <div className="mb-6 sm:mb-8">
        <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Replay Tested Transactions on Actual Networks</h2>
        <p className="text-gray-600 text-sm sm:text-base">
